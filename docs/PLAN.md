...
Release gates remain explicit: user A/B listening across the full permitted fixture matrix and Demucs profiles; frozen Windows x64 worker/package validation and performance checks; real permitted-song processing on the target Windows PC; and verification that packaged third-party notices are present. MelBand is not production-ready until these gates pass. The default supported source-duration range is 10 minutes; an operator can intentionally raise it with `KARAOKE_MAX_DURATION_SECONDS` when local policy and hardware permit.
